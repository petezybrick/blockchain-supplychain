...
public class SupplierTransactionOrcDao {
	private static final Logger logger = LogManager.getLogger(SupplierTransactionOrcDao.class);
	...
	public static SupplierTransactionOrcVo findByTemplate( SupplierTransactionOrcVo supplierTransactionOrcVo ) throws Exception {
		try( Connection con = PooledDataSource.getInstance().getConnection();
			...
			int offset = 1;
			// pstmt.setString(offset++, supplierTransactionOrcVo.getAttribute( );
			ResultSet rs = pstmt.executeQuery();
			if( rs.next() ) return new SupplierTransactionOrcVo(rs);
			else return null;
		} catch (Exception e) {
