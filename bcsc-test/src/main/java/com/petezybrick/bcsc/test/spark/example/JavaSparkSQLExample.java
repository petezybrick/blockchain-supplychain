...
public class JavaSparkSQLExample {
	private static String exampleFilesFolder;

	// $example off:create_ds$

	public static void main(String[] args) throws AnalysisException {
	  exampleFilesFolder = args[0];
    // $example on:init_session$
	  SparkConf conf = new SparkConf().setAppName("SVM Classifier Example");
		if( args.length > 1 ) conf.setMaster(args[1]);
	...
	private static void runBasicDataFrameExample(SparkSession spark) throws AnalysisException {
		// $example on:create_df$
		Dataset<Row> df = spark.read().json( exampleFilesFolder + "/people.json");
	...
	private static void runDatasetCreationExample(SparkSession spark) {
		...
		String path = exampleFilesFolder + "/people.json";
	...
	private static void runInferSchemaExample(SparkSession spark) {
		...
		JavaRDD<Person> peopleRDD = spark.read().textFile(exampleFilesFolder + "/people.txt")
				.javaRDD().map(line -> {
	...
	private static void runProgrammaticSchemaExample(SparkSession spark) {
		...
		JavaRDD<String> peopleRDD = spark.sparkContext()
				.textFile(exampleFilesFolder + "/people.txt", 1).toJavaRDD();
